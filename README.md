...
- მონაცემები არის არაბალნსირებული — ყველა დეპარტამენტი ყველა მაღაზიაში არ არსებობს.
...
- სადღესასწაულო კვირების წონა 5-ჯერ მეტია სტანდარტულ კვირაზე (WMAE-ს განმარტება).
- MarkDown სვეტებში ბევრი missing მნიშვნელობაა — დავამუშავეთ როგორც missing indicator + fillna(0).
...
- **Lag features:** ზუსტი თარიღით აგებული  — lag_1, 2, 4, 13, 26, 51, 52, 53.
...
**მნიშვნელოვანი აღმოჩენა (feature availability bug):** საწყის ვერსიაში `lag_1/2/4/13/26` ვიყენებდით feature-ებად, თუმცა აღმოვაჩინეთ, რომ ეს lag-ები რეალურ Kaggle test set-ზე NaN-ია უმეტეს რიგში — რადგან 1-26 კვირით უკან საჭირო გაყიდვების მონაცემი თავად test-ის პერიოდშივე ხვდება, სადაც რეალური გაყიდვები არ გვაქვს (არ ვიყენებთ autoregression-ს). + ეს იწვევდა train/production შეუსაბამობას: validation WMAE ძალიან კარგი გამოდიოდა (~1300), მაგრამ რეალურ Kaggle submission-ზე მივიღეთ **5961** — რადგან internal validation split თავად train-ის შიგნით ხდება, სადაც ეს lag-ები ყოველთვის ხელმისაწვდომია. გამოსწორების შემდეგ (მოკლე lag-ების გამორიცხვა feature ნაკრებიდან) მივიღეთ რეალისტური, production-თან თანმიმდევრული შედეგები.

---

## 4. მეთოდოლოგია

- **შეფასების მეტრიკა:** Weighted Mean Absolute Error (WMAE), holiday კვირების წონაა 5x.
...
- იგივე feature selection ლოგიკა, `tree_method='hist'`
...
- **გაუმჯობესება საუკეთესო ცალკეულ მოდელთან შედარებით (XGBoost):** 7.09
...
- **Order:** ფიქსირებული (1,1,1), **არ ჩატარებულა** per-series search — დავალების ინსტრუქციის მიხედვით, search საათობით გახანგრძლივებდა ტრენინგს 3,331 სერიაზე.
...
** მნიშვნელოვანი შენიშვნა:** ცხრილში მოცემული baseline-ის მნიშვნელობები განსხვავდება notebook-ების მიხედვით, + რადგან თითოეული მოდელი ტესტირებულია სხვადასხვა population-ზე (სრული dataset vs sample). **პირდაპირ შედარებადია** მხოლოდ ისინი, ვისაც იდენტური baseline (1789.91) აქვს — ესენია LightGBM, XGBoost, Ensemble, Prophet, DLinear, N-BEATS, PatchTST.
...
მხოლოდ საუკეთესო არქიტექტურაა რეგისტრირებული DagsHub Model Registry-ში, `WalmartSalesForecast` სახელით — **Ensemble (XGBoost 0.7 + LightGBM 0.3)**. ყველა დანარჩენი მოდელის pipeline ასევე დალოგილია MLflow-ში (`{Model}_Final_Model` run-ის ქვეშ), მაგრამ `REGISTER_AS_GLOBAL_BEST = False`.
...
2. **Deep Learning მოდელებმა (DLinear, N-BEATS, PatchTST) ვერ აჯობეს tree-based მოდელებს** — მოსალოდნელი შედეგია მცირე dataset-ის ზომის გამო (~143 კვირა თითო სერიაზე), რაც არასაკმარისია DL მოდელისთვის, რომ საკუთარი თავით ისწავლოს ტემპორალური სტრუქტურა hand-crafted feature-ების გარეშე.
